...
current_scaling = 12; %current loop needs to be faster
...
gr = 26/1; %gearhead reduction (motor speed / load speed)
ticks_per_mm = 327.6;
ticks_per_rev = 42;
grtot = 2*pi*ticks_per_mm/ticks_per_rev; %total gear ratio [rad/mm]
Klt = gr/grtot; %gearhead to load ratio [mm/rad]
...
Kt = 46.1 *(1e-3); %torque constant [Nm/A]
Kv = 207 *(2*pi/60); %speed constant [rad/(s*V)]
Ke = Kt; %back-emf constant [V*s/rad]
Jm = 23 *(1e-7) + Jgb + Jlm; %motor inertia [Kg*m2]
wo = 7390 *(2*pi/60); %no load speed [rad/s]
wn = 6450 *(2*pi/60); %nominal speed [rad/s]
Umax = 48; %input voltage [V]
Un = 36; %nominal voltage
Imax = 8.0; %max current [A]
In = 2.74; %nominal current [A]
...
wD = 6*w_c;  % cutoff frequency of second pole (NOTE: reduce scaling to limit high frequency vibrations)
Ti = 6/w_c;
