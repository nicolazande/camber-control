...
% Maxon GP42C 26:1, 1.5:1 spur ratio
gr = 12; %26/1; %gearhead reduction (motor speed / load speed)
ticks_per_mm = 327.6;
ticks_per_rev = 42;

%% load (motor side)
Jlm = 9.1 *(1e-6);
Jgb = 9.1 *(1e-7); %gearbox inertial motor side [Kg*m2]
% Klt = 1/(1000*2*pi*174.72*6/(48*91)); %gearhead to load ratio [m/rad]
Klt = 1/(1000*2*pi*ticks_per_mm/(ticks_per_rev*gr)); %gearhead to load ratio [m/rad]
% grtot = 1000*2*pi*174.72/48; %total gear ratio
grtot = 1000*2*pi*ticks_per_mm/ticks_per_rev; %total gear ratio

%% plant (motor)
% Maxon ECi40 70W 36V High-Torque
L = 0.512 *(1e-3); %inductance [H]
R = 0.582; %resistance [Ohm]
Kt = 24.1 *(1e-3); %torque constant [Nm/A]
Kv = 397 *(2*pi/60); %speed constant [rad/(s*V)]
Ke = Kt; %back-emf constant [V*s/rad]
Jm = 23 *(1e-7) + Jgb; %motor inertia [Kg*m2]
no = 4710 *(2*pi/60); %no load speed [rad/s]
Uin = 12; %48; %input voltage [V]
Io = 205 *(1e-3); %no load current [A]
wo = 4110 *(2*pi/60); %no load speed [rad/s]
Umax = 12; %36; %nominal voltage [V]
Imax = 20; %7; %max current [A]
In = 19.4; %2.74; %nominal current [A]
tauGp = 0.632 *(1e-3); %mechanical time constant [s] 
...
Kip_si = 60; %25; %integral gain [A/(rad*s)]
Kpp_si = 2.5; %proportional gain [A/rad]
Kdp_si = 0.025; %derivative gain [A*s/rad] (0.11*Kup*Tup)
